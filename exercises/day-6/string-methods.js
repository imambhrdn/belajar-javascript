...
// length adalah property, bukan method, jadi tidak pakai ()
...
// Harus pakai () karena ini adalah method/function
...
// Spasi di tengah tidak akan dihapus
...
// end tidak inklusif (tidak ikut)

let fullName = "Budi Santoso";

// Ambil nama depan (dari index 0 sampai index 4)
// Index: 0=B, 1=u, 2=d, 3=i, 4=  (spasi), jadi ambil 0-4
...
// Lebih aman digunakan jika tidak butuh kemampuan negative index
...
// Hanya yang pertama, tidak semua
...
let capitalizedWords = words.map(word =>
    // Ambil karakter pertama (index 0) jadi besar
    // Ambil sisa karakter (dari index 1) jadi kecil
...
// Balik kata tersebut
// split("") -> pecah per huruf
// reverse() -> balik array
// join("") -> gabung jadi string lagi
let reversed = word.split("").reverse().join("");

// Cek apakah palindrome (case insensitive)
...
function checkPasswordStrength(password) {
    // Cek minimal panjang 8
    let hasMinLength = password.length >= 8;

    // Jika password sama dengan lowercase, berarti tidak ada huruf besar
    let hasUpperCase = password !== password.toLowerCase();

    // Jika password sama dengan uppercase, berarti tidak ada huruf kecil
    let hasLowerCase = password !== password.toUpperCase();

    // Regex \d mencari digit (angka)
...
console.log("Is strong?", strength.isStrong); // Expected: true
